...
| UI                 | lucide-react-native 1.x (icons), @gorhom/bottom-sheet 5.x, expo-clipboard, expo-linear-gradient, expo-haptics, rn-emoji-keyboard 1.x, emojibase-data 17.x | Design system components in `components/ui/`; the native emoji keyboard supports English/Russian search and Android navigation-bar insets |
...
| Notifications      | expo-notifications 0.32.x, expo-task-manager 14.x, react-native-permissions                                                                               | Derived bounded reminder queue, native Android/iOS quick actions, platform-specific triggers, Android alarm permissions                   |
| Time Change Bridge | `@niqp/react-native-android-time-change`                                                                                                                  | npm package used only by Android-specific entrypoints                                                                                     |
...
| `utils/`      | Pure utility functions: `date.ts` (dayjs helpers), `storage.ts`/`storageIds.ts` (MMKV adapter and store IDs), notification scheduler/response utilities (`notifications.ts`, `reminderQueue.ts`, `reminderScheduler.ts`, `reminderScheduleLedger.ts`, `reminderResponseLedger.ts`, `reminderNotificationResponse.ts`, `reminderNotificationTask.ts`, `notificationQa.ts`), `fileOperations.ts`, `statsUtils.ts`. |
...
- Native habit persistence uses the `yaht-persistence` MMKV instance; runtime notification ledgers use the separate `yaht-runtime` MMKV instance. Keep JS and native storage IDs in sync with `utils/storageIds.ts` and the native reminder templates.
...
3. `utils/reminderScheduleLedger.ts` stores a plain MMKV-backed operational ledger of scheduled notification IDs and signatures in `yaht-runtime`; it is not part of the React-facing habit store in `yaht-persistence`.
...
6. Reminder `Done` and `Snooze` quick actions are handled by shared response logic in `utils/reminderNotificationResponse.ts` for foreground/JS fallback paths; Android background/terminated action taps can still bridge through the Expo TaskManager task in `utils/reminderNotificationTask.ts` when native handling delegates.
...
9. iOS foreground reminder notifications with the native habit action category intentionally present as banners/lists through the native delegate so simulator/device QA can exercise the same `Done`/`Snooze` actions while the app is active.
...
The app consumes `@niqp/react-native-android-time-change` from npm. Do not add the package as a Metro `watchFolders` entry or a local sibling `file:` dependency. YAHT uses a custom `index.js` entrypoint to preserve Expo Router startup while delegating Android Headless JS registration to `utils/registerTimeChangeTask.android.ts`; the default `utils/registerTimeChangeTask.ts` is a non-Android no-op so iOS/web builds do not import the Android native package. App-owned handling lives in `utils/timeChange.ts` and the Android-only `hooks/useTimeChangeManager.android.ts`; the default `hooks/useTimeChangeManager.ts` is a no-op for non-Android platforms. The native package only transports normalized `TIME_SET`, `TIMEZONE_CHANGED`, and optional API 37+ `TIMEZONE_OFFSET_CHANGED` events into JS. Keep habit, timer, reminder, and scheduling policy in YAHT code, not in the library.
...
- Root layout provides React Navigation `ThemeProvider` and `SafeAreaProvider`; hidden-header tab navigator scenes stay
  full-window, and individual tab screens apply top/side safe-area padding to their content so in-scene overlays such as
  bottom-sheet backdrops can cover the full window.
- `app/add/` is a modal nested native-stack flow with optional `habitId` search param for edit mode. Draft add/edit values live in the non-persisted `store/addHabitDraftStore.ts` so native child routes share form state.
