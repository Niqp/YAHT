...
import {
  drainIosNativeReminderActions,
  setIosNativeReminderActionStorageForTests,
} from "@/utils/iosNativeReminderActions";
...
const createStorage = (records: unknown[]) => {
  const values = new Map([[IOS_NATIVE_REMINDER_ACTION_STORAGE_KEY, JSON.stringify(records)]]);
  return {
...
describe("drainIosNativeReminderActions", () => {
  ...
  it("mirrors a native Done action into the in-memory Zustand store", async () => {
    ...
    setIosNativeReminderActionStorageForTests(storage);
    ...
    expect(storage.values.has(IOS_NATIVE_REMINDER_ACTION_STORAGE_KEY)).toBe(false);
  });
