...
7. Native background quick actions are installed by `plugins/with-native-reminder-actions.js`, which wraps the iOS plugin and copies Android Kotlin sources into the generated Android project. iOS uses a `UNUserNotificationCenterDelegate`; Android uses a higher-priority Expo notification event `BroadcastReceiver`. Both directly mutate the MMKV-backed Zustand habit store and cancel pending/delivered notifications in the tapped reminder series. Android writes only the main habit store plus the existing response/schedule ledgers, then JS rehydrates the main store on foreground/hydration before reconciliation.
