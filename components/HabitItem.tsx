	...
	useEffect(() => {
		if (habit?.completionType === "timed") {
			if (activeTimer && activeTimer.date === selectedDate) {
				...
				console.log(
					`HabitItem: Initializing active timer for habit ${habit.id} with baseTime: ${activeTimer.baseTime}s`,
				);
				setTimerActive(true);
			...
			displayTimeRef.current = setInterval(() => {
				...
				if (habit?.completionType === "timed") {
					...
					if (currentTime >= completionGoal && completionGoal > 0) {
						...
						console.log(
							`Timer reached goal during tick: ${currentTime}s >= ${completionGoal}s`,
						);
	...
	const handlePress = () => {
		...
		if (habit.completionType === "simple") {
			...
			if (isCompleted) {
				...
				setTimerActive(true);
				console.log(
					`Starting timer for habit ${habit.id} with baseTime: ${baseTime}s`,
				);
