...
| Notifications      | expo-notifications 0.32.x, expo-task-manager 14.x, react-native-permissions                                             | Derived bounded reminder queue, headless quick actions, platform-specific triggers, Android alarm permissions |
...
6. Reminder `Done` and `Snooze` quick actions are handled by shared response logic in `utils/reminderNotificationResponse.ts`; Android background/terminated action taps are bridged through the Expo TaskManager task in `utils/reminderNotificationTask.ts`, while iOS background taps use an early native response listener plus the mounted hook path.
7. `utils/reminderResponseLedger.ts` is a dedicated MMKV-backed 48-hour de-dupe ledger for notification response keys. Use it before mutating habit state from notification responses so Headless JS and UI listeners cannot apply the same action twice.
